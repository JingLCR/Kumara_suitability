...
class GeoProcessing(object):
    ...
    def reproject_raster_3(self, dataset, outrst, refdataset):
        """
        ...
        src = gdal.Open(dataset)
        ref = gdal.Open(refdataset)
        

        cols = ref.RasterXSize # Raster xsize
        rows = ref.RasterYSize # Raster ysize
        ...
        dest = mem_drv.Create(outrst, cols, rows, 1, gdal.GDT_Float32,  ['COMPRESS=LZW'])
    
        # Set the geotransform
        dest.SetGeoTransform(ref.GetGeoTransform())
        dest.SetProjection(ref.GetProjection())
        
        #set No data value
        outband = dest.GetRasterBand(1)
        outband.SetNoDataValue(self.no_data)
        # Perform the projection/resampling 
        gdal.ReprojectImage(src, dest, src.GetProjection(), ref.GetProjection(), gdal.GRA_NearestNeighbour)
...
#        rst.array2Raster(newArray, refdataset, dataset)
    ...
    def ESRIVector2Raster(self, file, field_id, field_Name, raster_res, out_dir):
        ...
        try:
            if file.split('.')[-1].lower() == 'shp':
                ...
                gdb = self.strip_end(file, r'\{}'.format(layer_name))
    ...
    def Vector2Raster(self, file, field_id, field_Name, raster_res, out_dir):
        ...
        try:
            ...
            elif file.split('\\')[-2].split('.')[-1].lower() == 'gpkg':
                driver = ogr.GetDriverByName("GPKG")
                layer_name = file.split('\\')[-1]
                gpkg = self.strip_end(file, r'\{}'.format(layer_name))
    ...
    def PolygonToRaster(self, inLayer, outFile, attr, pixel_size, d_type):#, x_topleft=0, y_topleft=0):
        ...
        # if x_topleft != 0 and y_topleft != 0:
        #     x_min = x_topleft
        #     y_max = y_topleft
            
# =============================================================================
#         x_min = 1180104.503600
#         y_max = 6045159.935000
#         x_max = 2084504.503600
#         y_min = 4823259.935000
# =============================================================================

        x_min = 1089971.000800
        y_max = 6223163.985000
        x_max = 2092000.000100
        y_min = 4747978.916800
        
        # Create the destination data source
        x_res = int((x_max - x_min) / pixel_size)
        y_res = int((y_max - y_min) / pixel_size)
        
        target_ds = gdal.GetDriverByName('GTiff').Create(outFile, x_res, y_res, 1, d_type, ['COMPRESS=LZW'])
        ...
        # Rasterize
        gdal.RasterizeLayer(target_ds, [1], inLayer, options = ["ATTRIBUTE="+attr, "ALL_TOUCHED=TRUE"])
    ...
    def DataTypeConversion(self, x):
        return {
                ...
                'Integer64': gdal.GDT_Int32,
                'String': gdal.GDT_Float32,
